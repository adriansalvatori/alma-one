...
abstract class PageAbstract {
			...
			<div id="cfw_admin_page_header" class="fixed top-0 divide-y shadow z-50">
				...
				<div class="min-h-[64px] bg-white flex items-center pl-8 justify-between">
					<div class="flex items-center">
						<span>
							<?php echo file_get_contents( CFW_PATH . '/build/images/cfw.svg' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						</span>
	...
	public function output_form_open( $id = null ) {
		?>
		<form name="settings" id="<?php echo $id ? esc_attr( $id ) : 'cfw_settings_form'; ?>" action="<?php echo esc_attr( wc_clean( $_SERVER['REQUEST_URI'] ?? '' ) ); ?>" method="post">
			<?php
			...
			<div class="flex">
				...
				<div class="ml-3 flex-1 md:flex md:justify-between">
					<p class="text-sm text-gray-700">
						<?php echo wp_kses_post( sprintf( cfw__( 'A %s plan is required to access this feature.', 'checkout-wc' ), $required_plans ) ); ?> <?php echo wp_kses_post( sprintf( cfw__( 'You can upgrade your license in <a target="_blank" href="%1$s">Account</a>. For help upgrading your license, <a target="_blank" href="%2$s">click here.</a>', 'checkout-wc' ), 'https://www.checkoutwc.com/account/', 'https://kb.checkoutwc.com/article/53-upgrading-your-license' ) ); ?>
					</p>
						...
						<div class="ml-3 text-sm">
							...
							<?php if ( isset( $descriptions[ $option_value ] ) ) : ?>
								<p id="small-description" class="text-gray-500">
									<?php echo wp_kses_post( $descriptions[ $option_value ] ); ?>
								</p>
							<?php endif; ?>
						</div>
					</div>
			...
			<div class="mt-4 space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
				<?php foreach ( $options as $option_value => $option_label ) : ?>
					<div class="relative flex items-start">
						...
						<div class="ml-3 text-sm">
							<label for="<?php echo esc_attr( "{$setting}_{$option_value}" ); ?>" style="vertical-align: unset;" class="font-medium text-gray-700">
								<?php echo wp_kses( $option_label, cfw_get_allowed_html() ); ?>
							</label>
