...
class Advanced extends PageAbstract {
	...
	protected function get_experimental_settings() {
		...
		$this->output_checkbox_row(
			...
			cfw__( 'Enable Beta Version Updates', 'checkout-wc' ),
			cfw__( 'Allows automatic updates to the latest beta version.', 'checkout-wc' )
		);

		$settings_obj  = SettingsManager::instance()->get_settings_obj( true );
		$data_migrated = get_option( 'cfw_v80_data_migrated', false );
		$succeeded     = isset( $_GET['force-v8-data-remigrate-done'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended

		if ( ! empty( $settings_obj ) && $data_migrated && ! $succeeded ) {
			$url = add_query_arg(
				array(
					'page' => 'cfw-settings-advanced',
					'force-v8-data-remigrate'  => 'true',
					'nonce' => wp_create_nonce( 'force-v8-data-remigrate' ),
				),
				admin_url( 'admin.php' )
			);
			echo '<a id="cfw-remigrate-v7" href="' . esc_url( $url ) . '" class="button button-secondary">' . cfw_esc_html__( 'Remigrate v7 Settings to v8', 'checkout-wc' ) . '</a>';
		}

		if ( $succeeded ) {
			echo '<div style="color:red;"><p>' . cfw_esc_html__( 'Data migration from v7 to v8 complete. Your v8 settings should now match your previous v7 settings.', 'checkout-wc' ) . '</p></div>';
		}
	...
	public static function generate_settings_export() {
		...
		$values = $wpdb->get_results( "SELECT option_name, option_value FROM $wpdb->options WHERE option_name LIKE '\_cfw_%' AND option_name <> '_cfw__settings'" );

		foreach ( $values as $value ) {
			$settings[ $value->option_name ] = $value->option_value;
		}

		$settings['_cfw_logo_attachment_url'] = wp_get_attachment_url( $settings['_cfw_logo_attachment_id'] );
	...
	public function maybe_upload_settings() {
		...
		if ( ! is_null( $decoded ) && isset( $decoded['_cfw_logo_attachment_id'] ) && ! empty( $decoded['_cfw_logo_attachment_id'] && false !== $decoded['_cfw_logo_attachment_url'] ) ) {
			$image_upload                       = $this->upload_logo( $decoded['_cfw_logo_attachment_url'] );
			$decoded['_cfw_logo_attachment_id'] = $image_upload ? $image_upload : '';
		} else {
		...
		foreach ( $decoded as $key => $value ) {
			update_option( $key, maybe_unserialize( $value ) ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.serialize_unserialize
		}
