...
function cfw_maybe_show_already_have_an_account_text() {
	...
	<div class="cfw-have-acc-text cfw-small <?php echo WC()->checkout()->is_registration_required() ? 'account-does-not-exist-text' : ''; ?>">
		...
		<a id="cfw-login-modal-trigger" href="javascript:">
			 ...
			 * @since 2.0.0
			 */
			echo apply_filters( 'cfw_login_faster_text', esc_html__( 'Log in.', 'checkout-wc' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			?>
		</a>
		...
		<?php if ( wc_coupons_enabled() ) : ?>
			...
			<div class="cfw-promo-wrap <?php echo $hide_promo_code_by_default ? 'cfw-hidden' : ''; ?>">
				<div class="row cfw-promo-row cfw-input-wrap-row">
					...
					?>
					<div class="cfw-input-wrap cfw-button-input col-4 cfw-promo-code-button-wrap form-row">
...
function cfw_footer_content() {
	...
	if ( ! empty( $store_policies ) ) {
		?>
		<ul class="cfw-store-policies">
			<?php foreach ( $store_policies as $policy ) : ?>
				<?php
				if ( ! isset( $policy->page->id ) || empty( $policy->title ) ) {
					continue;
				}

				$policy_data = get_post( $policy->page->id );

				if ( ! $policy_data ) {
					return;
				}

				$title   = $policy_data->post_title;
				$content = cfw_apply_filters( 'the_content', $policy_data->post_content );
				?>
				<li>
					<a href="#policy_content_<?php echo esc_attr( $policy->page->id ); ?>" target="_blank">
						<?php echo esc_html( $policy->title ); ?>
					</a>
					<div id="policy_content_<?php echo esc_attr( $policy->page->id ); ?>" style="display:none">
						<div class="cfw-modal-header">
							<h2><?php echo esc_html( $title ); ?></h2>
						</div>

						<div class="cfw-modal-content">
	...
	<p>
		<?php
		if ( ! empty( $footer_text = SettingsManager::instance()->get_setting( 'footer_text' ) ) ) {
