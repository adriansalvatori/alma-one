...
class Guaven_woo_search_front
{
    ...
    public function personal_interest_collector()
    {
        if ( ($this->skip_transients() or is_singular('product')) and !empty($this->cookieprods) ) {
            ...
            if(!headers_sent())setcookie('guaven_woos_lastvisited', $uniqcookieid, time() + 86400, '/', "", 0);
        }
    }

    public function local_values(){

      $cached_guaven_woos_local_values=apply_filters('cached_guaven_woos_local_values',[]);
      if(!empty($cached_guaven_woos_local_values))return $cached_guaven_woos_local_values;
      ...
      $guaven_woos_local_values = array(
          ...
          'translit_data' => esc_attr(get_option('guaven_woos_translit_data')),
          ...
          'selector' => get_option('guaven_woos_selector') != '' ? $this->get_string(stripslashes(get_option('guaven_woos_selector'))) : '[name="s"]',
          'live_filter_selector' => get_option('guaven_woos_filter_selector') != '' ? $this->get_string(stripslashes(get_option('guaven_woos_filter_selector'))) : '',
          ...
          'expression_segments'=>get_option('guaven_woos_expression_segments')!=''?explode(",",esc_attr(get_option('guaven_woos_expression_segments'))):array('under','around','above'),
          ...
          'live_server'=>esc_attr(get_option("guaven_woos_live_server")),
          ...
          'live_ui_layout'=> esc_attr(get_option("guaven_woos_live_ui_layout")),
          'show_when_unfocus'=>get_option('guaven_woos_show_when_unfocus') != '' ? 1 : 0,
      );
      $trendnum=get_option('guaven_woos_data_trend_num');
      if (!empty($trendnum)) {
          $guaven_woos_data_trend                  = $this->trend_calculator();
          if(is_array($guaven_woos_data_trend[0]) and is_array($guaven_woos_data_trend[1])){
      ...
      do_action('guaven_woos_local_values_set',$enqname,$guaven_woos_local_values);
    ...
    function multi_currency_args($guaven_woos_local_values){
        ...
        elseif( defined('WOOMULTI_CURRENCY_VERSION')){
			$woo_multi_currency_params=get_option('woo_multi_currency_params');
			$wcums_cur_cur=!empty($_COOKIE['wmc_current_currency'])?$_COOKIE['wmc_current_currency']:$this->getcookie('wmc_current_currency');
			$foundkey='';
			if(!empty($wcums_cur_cur)){
				foreach($woo_multi_currency_params["currency"] as $curkey=>$currency){
					if($currency==$wcums_cur_cur){
						$foundkey=$curkey;
					}
				}
				if(!empty($foundkey)){
					$curprefix='';$curendfix='';
					if($woo_multi_currency_params['currency_pos'][$foundkey]=='right_space')$curprefix=' ';
					elseif($woo_multi_currency_params['currency_pos'][$foundkey]=='left_space')$curendfix=' ';
					$woo_multi_currency_params['currency_pos'][$foundkey]=str_replace("_space","",$woo_multi_currency_params['currency_pos'][$foundkey]);
					$guaven_woos_local_values['woo_multicurrency']=
						array(
						'position'=>$woo_multi_currency_params['currency_pos'][$foundkey],
						'symbol'=>$curprefix.html_entity_decode(get_woocommerce_currency_symbol()).$curendfix,
						'rate'=>$woo_multi_currency_params['currency_rate'][$foundkey],
						'conv'=>'',
						'charm'=>'',
						'round'=>''
					);
