...
class Guaven_woo_search_admin
{
    ...
    private function to_default_runner()
    {
        ...
        $this->is_checked('guaven_woos_show_when_unfocus', '');
    ...
    public function cron_token($tomatch='')
    {
        ...
        if($tomatch!='')return ($tomatch==$ret or $tomatch==$ret.'_trigger');
        return $ret;
    }
...
jQuery(".Rebuild-SearchBox-Cache a").attr("href","javascript://");
  ...
  <style>.Rebuild-SearchBox-Cache a {background:#008ec2 !important}</style>';
    ...
    public function cache_rebuild_cron_mode(){
        ...
        if(!$this->cron_token($this->argv[1]))return;

        if(strpos($this->argv[1],'_trigger')!==false) {
            global $wpdb;
            $is_there_todo=$wpdb->get_var("select option_value from $wpdb->options where option_name='_transient_gws_cron_rebuilder_should_run'");
            if(!empty($is_there_todo)){
                set_transient('gws_cron_rebuilder_should_run','',3600);
            }
            else {
        ...
        $GLOBALS["woos_cron_step_max"]=1000;//temp
        ...
        die(PHP_EOL.'Finished'.$GLOBALS["woos_cron_step_max"].PHP_EOL);
    }
    public function cache_rebuild_ajax_callback()
    {
        ...
        if (isset($this->argv[1]) and $this->cron_token($this->argv[1])) {
...
where meta_key='_sku' and b.post_type='product_variation' and b.post_parent=" . intval($value->ID));
            ...
            $products_array[$value->ID] =apply_filters('gws_manipulate_the_layout',$products_array[$value->ID] ,$_product,$key,$value);
    ...
    public function price_and_parparser($value, $langfixer, $_product, $guaven_woos_taxenabled, $title_and_hidden_sku, $permalink_structure, $thumb_quality)
    {
        ...
        foreach($displayed_brands as $dbs){
          ...
          $product_pwbbrabd = wp_get_post_terms($objid, trim($dbs));
    ...
    public function synonym_list($add_synonyms_too, $ptitle)
    {
        ...
        $synonym_list     = explode(',', str_replace("'","’",stripslashes($add_synonyms_too)));
    ...
    public function fs_or_db($dironly='')
    {
        ...
        return $cache_dir.'/guaven_woos_data'.GUAVEN_WOO_SEARCH_CACHE_ENDFIX.$this->get_current_language_code().'_processing.js';
    }
