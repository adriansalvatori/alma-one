...
<button class="tablinks" id="guaven_woos_tablink_backend" onclick="openSettingTab(event, 'guaven_woos_tab_backend');return false;"><?php _e('Backend Search','guaven_woo_search');?></button>
<button class="tablinks" id="guaven_woos_tablink_admin" onclick="openSettingTab(event, 'guaven_woos_tab_admin');return false;"><?php _e('Data Building','guaven_woo_search');?></button>
<button class="tablinks" id="guaven_woos_tablink_advanced" onclick="openSettingTab(event, 'guaven_woos_tab_advanced');return false;"><?php _e('Advanced Settings','guaven_woo_search');?></button>
<button class="tablinks" id="guaven_woos_tablink_updates" onclick="openSettingTab(event, 'guaven_woos_tab_updates');return false;"><?php _e('Getting Updates','guaven_woo_search');?></button>
<button class="tablinks" id="guaven_woos_tablink_faq" onclick="openSettingTab(event, 'guaven_woos_tab_faq');return false;"><?php _e('FAQ','guaven_woo_search');?></button>
</div>
...
?>>
<?php _e('Replace my theme\'s search results page with "SSSS" (Standalone Simple and Smart Search) module by this plugin.','guaven_woo_search');?>
<br> 
...
?>" class="small-text"> <?php echo  sprintf( __('Default value is %s'),10000);?>
  ...
  <td scope="row" class="howtorebuild">


  <?php
$guaven_woos_autorebuild = get_option("guaven_woos_autorebuild");
?>
  <label><?php _e('Auto-Rebuild via Scheduled Cron Jobs','guaven_woo_search');?></label>:<?php 
  if(defined( 'POLYLANG_VERSION' ) or defined( 'ICL_SITEPRESS_VERSION' )){
    $crcommand='<code>php '.ABSPATH.'index.php  '.$cron_token.'_trigger LANGUAGE_CODE</code>';
    echo $crcommand;
  }
  elseif(is_multisite()){
    $crcommand='<code>php '.ABSPATH.'index.php  '.$cron_token.'_trigger 0 SUBSITE_ID</code>';
    echo $crcommand;
  }
  else {
    $crcommand='<code>php '.ABSPATH.'index.php  '.$cron_token.'_trigger</code>';
    echo $crcommand;
  }
  ?>
  <br>  <br>
  <small>*<?php _e('To run the rebuilder command without checking for changes in the products, simply remove the <b>"_trigger"</b> suffix from the command.','guaven_woo_search');?></small>
  <br>

  <small>*<?php _e('The frequency for your scheduled cron job can be set to any interval you desire, with "5 minutes" being the lowest recommended frequency. The highest frequency that can be set is "daily", "weekly", etc.','guaven_woo_search');?></small>
  <br>

  <small>*<?php echo sprintf(__('An example command for "5 minutes" interval: %s','guaven_woo_search'),"*/5 * * * * ".strip_tags($crcommand));?></small>
  <br>
...
<br>
  ...
  ?>" class="small-text">
    ...
    <p>
    <label>
    <input name="guaven_woos_show_when_unfocus" type="checkbox" value="1" class="tog" <?php
echo checked(get_option("guaven_woos_show_when_unfocus"), 'checked');
?>>
    <?php _e('Don\'t close the search box after unfocus event.','guaven_woo_search');?> </label>
    </p>
    <small><?php _e('Not needed for the most cases. When to check this, the search box will be closed only after outside-click event.','guaven_woo_search');?></small>
    </p>
...
<input type="hidden" id="gvs_active_tab" name="gvs_active_tab" value="">
...
function openSettingTab(evt, cityName) {
    ...
    jQuery("#gvs_active_tab").val(cityName.replace("tab_","tablink_"));
}
<?php 
$currentab='guaven_woos_tablink_live';
if(!empty($_POST["gvs_active_tab"]) and in_array($_POST["gvs_active_tab"],
  ['guaven_woos_tablink_live',
  'guaven_woos_tablink_backend', 
  'guaven_woos_tablink_admin', 
  'guaven_woos_tablink_advanced',
  'guaven_woos_tablink_updates',
  'guaven_woos_tablink_faq']
  )){
    $currentab=esc_attr($_POST["gvs_active_tab"]);
  }
?>
document.getElementById("<?php echo $currentab;?>").click();
</script>
